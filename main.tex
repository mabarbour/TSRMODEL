...
In Figure 3 of \citet{Gilbert2014}, only $K$ depends on temperature; the rest are held constant. 
If we consider all of the additional dependencies discussed by \citet{Gilbert2014}
...
Comparing with Figure 3 in \cite{Gilbert2014}, we see that adding temperature dependence in $r$, $a$, and $m$ causes $\hat{C}:\hat{R}$ to decline with temperature (instead of increasing) and stability to decrease at a slower rate with increasing temperature.
...
Further, the TSR facilitates an increase in system stability with warming, relative to a decrease in stability without it. 
...
Deviations in the expected response of consumer-resource dynamics to temperature with the TSR, compared to a system without it, reflect the body mass-dependence of demographic parameters. 
...
%add: compare things like r where direct and indirect act in same direction to things like attack rate, where act in opposite direction

%STRENGTHS AND ASYMMETRIES
Although there are a number of theoretical explanations for the TSR \citep{Berrigan1994,Perrin1995,VanderHave1996,Angilletta2003,DeLong2012}, much of its support is empirical, and rates of decline are not yet predictable a priori.
...
Our theoretical results show that these empirically derived responses can have large effects on system stability.
%add something about delong's 2% vs 30% argument?
In addition, when we allowed larger (consumer) organisms to experience a greater decline in body size with warming, and thus a temperature-dependent consumer-resource body size ratio, effects of the TSR were further amplified relative to a model with symmetric declines in body size.
...
Consumer-resource interactions can impose selection on body size \cite{Abrams1996}, and this selection may vary over a thermal gradient. 
